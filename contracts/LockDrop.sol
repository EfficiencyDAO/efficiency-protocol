...
contract LockDrop is Exponential, ReentrancyGuard {
    ...
    */
    ...
    constructor(string memory name_, 
                address ctoken_, 
                uint256 claimUnlockTime_) public {
        
        require(claimUnlockTime_ > now, "claim unlock time is before current time");
        name = name_;
        ctoken = ctoken_;
    ...
    function deposit(uint256 amount) external nonReentrant returns (bool) {
        ...
        bool transferStatus = ct.transferFrom(msg.sender, address(this), amount);
    ...
    function claim() external nonReentrant {
        ...
        CToken ct = CToken(ctoken);
        bool transferStatus = ct.transfer(msg.sender, accountBalances[msg.sender]);
    ...
    function setOwner(address newOwner) external onlyOwner returns (bool) {
        ...
        require(newOwner != address(this), "DANGER: Attempted to throw away ownership");
        owner = newOwner;
    }
}